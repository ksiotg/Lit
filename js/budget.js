...
function buildYear(){
  ...
  for(let m=0;m<12;m++){
    ...
    const remain=fi-fe;
    row.innerHTML=`<span style="font-size:12px;font-weight:700;color:${m===curM?'var(--budget)':'var(--muted)'};width:28px;flex-shrink:0;">${m+1}월</span><div style="flex:1;min-width:0;"><div style="display:flex;justify-content:space-between;gap:6px;font-size:11px;margin-bottom:3px;"><span style="color:var(--income);font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">+${fi.toLocaleString('ko-KR')}</span><span style="color:var(--expense);font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">-${fe.toLocaleString('ko-KR')}</span></div><div style="height:4px;background:#f0f0f5;border-radius:99px;overflow:hidden"><div style="height:100%;width:${fi?Math.min(100,Math.round(fe/fi*100)):0}%;background:var(--budget);border-radius:99px"></div></div></div><div class="year-remain-pill"><span style="color:${remain>=0?'var(--budget)':'var(--expense)'}">${signFmt(remain)}</span></div>`;
