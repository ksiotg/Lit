...
// 월간 회고를 쓰기 전에, 그 달에 작성한 일별 회고들을 질문별로 모아서 한눈에 훑어볼 수 있게 해줌.
// "1일: 답변 / 2일: 답변 / ..." 형태로 질문마다 묶어서 보여주고, 자유 메모도 같은 방식으로 모아줌.
function buildMonthlyReviewDigest(y,m){
    const dim=new Date(y,m+1,0).getDate();
    const byQuestion={};
    REVIEW_QUESTIONS.forEach(q=>{byQuestion[q.id]=[];});
    const memos=[];
    let reviewCount=0;
    for(let d=1;d<=dim;d++){
          const rv=S.getReview(y,m,d);
          if(!rv)continue;
          reviewCount++;
          REVIEW_QUESTIONS.forEach(q=>{
                  const a=rv.answers?.[q.id];
                  if(a&&a.trim())byQuestion[q.id].push({d,a:a.trim()});
          });
          if(rv.memo&&rv.memo.trim())memos.push({d,a:rv.memo.trim()});
    }
    const wrap=mkDiv('');wrap.style.cssText='padding:0 16px 4px;';
    if(reviewCount===0){
          wrap.innerHTML='<div class="empty">이번 달 작성한 일별 회고가 아직 없어요</div>';
          return wrap;
    }
    const dayLine=({d,a})=>{
          const line=mkDiv('');line.style.cssText='font-size:12px;line-height:1.55;';
          const dEl=document.createElement('span');dEl.style.cssText='font-weight:800;color:var(--review);';dEl.textContent=`${d}일`;
          const aEl=document.createElement('span');aEl.style.cssText='color:var(--text);';aEl.textContent=' · '+a;
          line.appendChild(dEl);line.appendChild(aEl);
          return line;
    };
    const body=mkDiv('');
    body.style.cssText='max-height:38vh;overflow-y:auto;display:flex;flex-direction:column;gap:14px;background:var(--bg);border-radius:12px;padding:12px 14px;margin-bottom:10px;';
    REVIEW_QUESTIONS.forEach(q=>{
          const list=byQuestion[q.id];
          if(!list.length)return;
          const block=mkDiv('');
          const label=mkDiv('');label.style.cssText='font-size:11.5px;font-weight:700;color:var(--muted);margin-bottom:6px;';
          label.textContent=`${q.emoji} ${q.text}`;
          block.appendChild(label);
          const lines=mkDiv('');lines.style.cssText='display:flex;flex-direction:column;gap:4px;';
          list.forEach(item=>lines.appendChild(dayLine(item)));
    ...
    if(memos.length){
          const block=mkDiv('');
          const label=mkDiv('');label.style.cssText='font-size:11.5px;font-weight:700;color:var(--muted);margin-bottom:6px;';
          label.textContent='📝 자유 메모';
          block.appendChild(label);
          const lines=mkDiv('');lines.style.cssText='display:flex;flex-direction:column;gap:4px;';
          memos.forEach(item=>lines.appendChild(dayLine(item)));
          block.appendChild(lines);
          body.appendChild(block);
    }
    const toggleBtn=document.createElement('button');
    toggleBtn.type='button';toggleBtn.className='freq-btn active';
    toggleBtn.style.cssText='width:100%;margin-bottom:10px;';
    toggleBtn.innerHTML=`${icon('list',13)} 이번 달 기록 모아보기 (${reviewCount}일 작성)`;
    toggleBtn.onclick=()=>{
          const nowOpen=body.style.display!=='none';
          body.style.display=nowOpen?'none':'flex';
...
function openMonthlyReviewPopup(){
    ...
    const digestWrap=document.getElementById('monthlyReviewDigest');digestWrap.innerHTML='';
    digestWrap.appendChild(buildMonthlyReviewDigest(rvY,rvM));
